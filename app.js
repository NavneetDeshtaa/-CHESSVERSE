...
io.on("connection", (socket) => {
  ...
  if (!players.white) {
    ...
    socket.emit("playerRole", { role: "w", message: "You are White. Waiting for other player…" });
    socket.emit("gameMessage", "Waiting for Black to join…");
  } else if (!players.black) {
    players.black = socket.id;
    socket.emit("playerRole", { role: "b", message: "You are Black. Waiting for White to move…" });
    io.to(players.white).emit("gameMessage", "Black has joined. Your turn – Make your move!");
  } else {
  ...
  socket.on("move", (move) => {
    ...
    if (!isPlayerTurn) return socket.emit("gameMessage", "It is not your turn.");

    const result = chess.move({ from: move.from, to: move.to, promotion: "q" });
    if (!result) return socket.emit("gameMessage", "Invalid move. Try again.");
    ...
    if (nextPlayer) io.to(nextPlayer).emit("gameMessage", "Your turn – Make your move!");
    if (waitingPlayer) io.to(waitingPlayer).emit("gameMessage", "Waiting for opponent's move…");
    ...
    if (chess.game_over()) {
      if (chess.in_checkmate()) {
        ...
        if (chess.in_stalemate()) reason = "Game ended in a stalemate!";
        else if (chess.insufficient_material()) reason = "Draw due to insufficient material!";
        else if (chess.in_threefold_repetition()) reason = "Draw by threefold repetition!";
        else if (chess.half_moves >= 100) reason = "Draw by the 50-move rule!";
  ...
  socket.on("resetGameRequest", () => {
    ...
    if (otherPlayer) io.to(otherPlayer).emit("resetRequest");
  });
